...
abstract class YamlCE {
  const DIMENSION_REGEX = '~^(.*?)/(.*)$~';

  public static function parse(array $data, Config $config = null): Config {
    $_config = $config ? clone $config : new Config();
    $dimensions = static::parseDimensions($data, $_config->getDimensions());
    $properties = static::parseProperties($data, $dimensions);
    $_config->setDimensions($dimensions);
    $_config->setData($properties);
    return $_config;
  }
  ...
  protected static function parseDimensions($data, $dimensions = []) {
    foreach ($data as $key => $initializers) {
      ...
      if (preg_match(self::DIMENSION_REGEX, $key, $matches)) {
        $dimension = static::createDimension(
          $matches[1],
          $matches[2],
          $initializers,
          $dimensions
   ...
   * @return Dimension
   */
  protected static function createDimension(
    $name,
    $brackets,
    $initializers,
    array $dimensions
    ...
    if (
      !is_string($brackets) ||
      mb_strlen($brackets) % 2 != 0 ||
      mb_strlen($brackets) == 0
    ) {
      throw new YamlElcException(
        "Dimension definition error: brackets string must be symmetrical ($name:$brackets)."
      );
    }
    ...
    foreach ($dimensions as $dimension) {
      if ($brackets == $dimension->brs()) {
        throw new YamlElcException(
          "Dimension definition error: brackets ambiguous ($name:$brackets)."
        );
      }
    }
    // Parse declarations
    [$items, $conditions] = static::parseDimensionItemRecursive(
      $initializers,
      $dimensions
    );
    $depends = [];
    ...
    $depends_dimensions = array_intersect_key(
      $dimensions,
      array_flip(array_unique($depends))
    );
   ...
   * @return array
   */
  protected static function parseDimensionItemRecursive(
    $data,
    array $dimensions
  ) {
    $items = [];
    ...
    foreach ($data as $key => $value) {
      [$domain_expr, $key_conditions] = static::parseKeyExpression(
        $key,
        $dimensions
      );
      if (is_array($value) && static::isAssoc($value)) {
        [$sub_items, $sub_conditions] = static::parseDimensionItemRecursive(
          $value,
          $dimensions
        );
        foreach ($sub_items as $sub_id => $sub_item) {
          $items[] = $sub_item;
          $conditions[] = static::mergeConditions(
            $key_conditions,
            $sub_conditions[$sub_id]
          );
        }
        ...
        // Error, must be array
        throw new YamlElcException(
          "Dimension value must be an array: '$value'"
        );
      }
      if ($domain_expr !== "-") {
        try {
          [$register, $domain] = static::parseDomainExpression($domain_expr);
   ...
   * @return DataArray|DataMap
   */
  protected static function parsePropertiesRecursive(
    $data,
    $dimensions,
    $source_reference = ""
  ) {
    ...
    foreach ($data as $key => $value) {
      ...
      [$name, $conditions] = static::parseKeyExpression($key, $dimensions);
      $name = static::normalizeKey($name);
      if (is_array($value)) {
        $below = static::parsePropertiesRecursive(
          $value,
          $dimensions,
          $reference
        );
    ...
    foreach ($filters as $filter_string) {
      if (preg_match('~^(.+?)=(.+)$~', $filter_string, $matches)) {
        ...
        if (!isset($dimensions[$dimension_name])) {
          throw new YamlElcException(
            "Filter error: dimension '$dimension_name'' is unknown"
          );
        }
        try {
        ...
        throw new YamlElcException(
          "Filter error: incorrect format, filter: '$filter_string'"
        );
      }
    }
    return $conditions;
  ...
  protected static function parseKeyExpression($key, array $dimensions) {
    ...
    foreach ($dimensions as $dimension_name => $d) {
      ...
      while (true) {
        ...
        if ($state !== "EXIT") {
          throw new YamlElcException(
            "Parse error (key='$key' dimension='$dimension_name')"
          );
        }
        ...
        if (trim($condition_expression) !== "") {
          try {
            $conditions = array_merge(
              $conditions ?? [],
              static::parseConditionExpression($condition_expression, $d)
            );
          } catch (YamlElcException $e) {
    ...
    if ($end_of_name_position < count($chars)) {
      throw new YamlElcException(
        "Dimension definition error: garbage at the end of the key: '$key'.\nOriginal key: $key\nKey dimensions:\n{static::serializeArray($conditions)}"
      );
    }
  ...
  protected static function parseDomainExpression($domain_expression) {
    if (strpos($domain_expression, ",") !== false) {
      throw new YamlElcException(
        "Initializer key must contain only one domain"
      );
    }
    $parts = array_filter(explode(":", $domain_expression));
    if (count($parts) > 1) {
      throw new YamlElcException(
        "Initializer key must contain only one register"
      );
    }
   ...
   * @return array
   */
  protected static function parseConditionExpression(
    $condition_expression,
    Dimension $dimension
  ) {
    $conditions = [];
    ...
    foreach ($parts as $register => $value) {
      if (
        ($resolved_values = $dimension->resolveValue($value, $register)) !==
        false
      ) {
        $values[$register] = array_merge(
          $values[$register] ?? [],
          $resolved_values
        );
      } else {
        throw new YamlElcException(
          "Domain or value not found: '$value', register: '$register'"
        );
      }
    }
